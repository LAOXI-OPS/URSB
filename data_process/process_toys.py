"""处理 Amazon Toys_and_Games 数据集为 SBCDR 项目格式。

用法: python process_toys.py
输出: data/toys_and_games/train.df, val.df, test.df
"""
import json
import pandas as pd
import numpy as np
...
JSONL_PATH = Path("F:/迅雷下载/Toys_and_Games.jsonl")
OUTPUT_DIR = Path("E:/code/reproduce/my_proj/SBCDR_new/data/toys_and_games")
TARGET_USERS = 43548
RANDOM_SEED = 42
...
with open(JSONL_PATH, 'r', encoding='utf-8') as f:
    for i, line in enumerate(f):
        ...
        if obj["rating"] >= 4.0:
            uid = obj["user_id"]
            aid = obj["asin"]
            ts = obj["timestamp"]  # 毫秒时间戳
            user_items[uid].append((aid, ts))
            item_set.add(aid)
        if (i + 1) % 2000000 == 0:
