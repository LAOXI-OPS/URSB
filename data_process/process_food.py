"""处理 Amazon Grocery_and_Gourmet_Food 数据集为 SBCDR 项目格式。

用法: python process_food.py
输出: data/food/train.df, val.df, test.df
"""
import json
import pandas as pd
import numpy as np
...
JSONL_PATH = Path("F:/迅雷下载/amazon数据集/Grocery_and_Gourmet_Food.jsonl")
OUTPUT_DIR = Path("E:/code/reproduce/my_proj/SBCDR_new/data/food")
TARGET_USERS = 62512
MIN_INTERACTIONS = 9
RANDOM_SEED = 42
...
print(f"[2/4] 过滤交互数 >= {MIN_INTERACTIONS} 的用户...")
user_items = {uid: items for uid, items in user_items.items() if len(items) >= MIN_INTERACTIONS}
